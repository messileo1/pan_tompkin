...
def get_annotation_count(path, dat_name):
    ...
    # print(f"   所有标注点总数 (含节奏标志): {len(annotation.sample)}")
    print(f"   实际心跳总数 (Beats): {len(beats)}") # 以这个为主
    return len(beats)
...
if __name__ == '__main__':
    ...
    data_list = [100,101,102,103,104,105,106,107,108,109,111,112,113,114,115,116,
    ...
    for i in range(len(data_list)):
        dat_name = str(data_list[i]) + '.dat'

        # ecg_data1, ecg_data2 = rd.read_f16(path, dat_name)
        ecg_data1, ecg_data2 = rd.read_f212(path, dat_name) # ecg_data1为 MLII 导联，R波明显
        ...
        # end_idx = end_sec * fs
        # ecg = ecg_data1[start_idx:end_idx]

        ecg = ecg_data1[:]  # 也可以读取原始信号的全部信号
        ...
        print(f"召回率为: {len(qrs_idx_flt)/all_beats}")
